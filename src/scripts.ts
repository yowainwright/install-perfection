...
export function resolveJSON(path: string, debug = false): InstallPerfectionJson {
  try {
...
export function configureDependencyList({
  dependencies = {},
  include = {},
  ignore = [],
}: ConfigureDependencyListOptions = {}) {
  ...
  const deps = hasDependencies
    ? dependencyNames
        .filter((dep: string) => !(ignore as string[]).includes(dep))
        .map((name: string) => ({ name, version: dependencies[name] }))
    : []
...
export const configureDepsToInclude = (
  { include: configInclude = {} }: Config,
  { include: unmergedInclude = [] }: Options,
) => {
  const optionInclude = unmergedInclude?.length
    ? unmergedInclude.reduce(
        (acc: Record<string, string>, curr: string) => ({ ...acc, ...(JSON5.parse(curr) as Record<string, string>) }),
        {},
      )
    : {}
...
export const configureDepsToIgnore = (
  { ignore: configIgnore = [] }: Config,
  { ignore: optionsIgnore = [] }: Options,
) => [...configIgnore, ...optionsIgnore]
